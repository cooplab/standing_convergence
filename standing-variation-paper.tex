...
\newcommand{\gc2}[1]{{\it\color{purple}(#1)} }
...
deficiency alleles thought to confer resistance to malaria, \gc2{where the large
mutational target size of G6PD makes likely candidate for adaptation
...
\gc2{consistent with the view that they arose from a combination of} from standing variation and new mutations since the advent of malaria.
Our results suggest that much of adaptation may be geographically local 
even when selection pressures are widespread. \gc2{Therefore, we argue
...
In some cases this convergent evolution \gc2{is the result of quite
...
and to \gc2{what degree species should be regarded} as cohesive units. 
...
inference in asexuals \citep{martens_interfering_2011}.}
...
\begin{figure}[ht]
  ...
  \end{center}
  \caption{
{\bf Simulation of the geographic spread of new mutations}. Simulation on a grid of $101 \times 101$ populations with 100 individuals in each, showing spread of a beneficial allele with advantage $s=0.1$.  Times are in numbers of generations, with probability of mutation per generation 0.3. 
}
...
parameter \gc2{comprised of} our parameters of interest. \gc{In
...
and that widespread selective sweeps \gc2{should tend to} occur
...
homozygote for the mutant alleles. \gc2{Note that this means that we
...
is $\sigma^2$. \gc2{Note that we assume that the dispersal kernel is
...
but may persist for \gc2{a short} time (note that we have assumed that $s_d$ is not too small).
...
\gc2{Our $\lambda_0$ is the mean density
...
allele at mutation selection balance $\mu/s_d$
\citep{Haldane:27,Haldane:37, Orr:01}.
...
and spread through human populations in areas where malaria \gc2{has been} endemic \citep{Kwiatkowski:05}. 
...
Another particularly impressive case of convergent evolution \gc2{is} the numerous changes throughout the X-linked G6PD gene, 
...
Given \gc2{such strong selection the alleles} should have risen
...
\gc2{to} escape drift and become established. 
...
as in that case the speed of spread does not matter \gc2{only the
...
fitness cost of G6PD-deficiency alleles before malaria ($s_d$) was \gc2{high}. 
...
the typical patch (i.e.\ with distribution given by the Palm measure)
occupies area $1/(\lambda_0 + \nu_+)$. \gc{NEED REF FOR PALM}
...
\gc2{Given that it has not been reached by $t$, the probability} that it will be reached by time $t+dt$ 
...
\end{equation}
\gc2{this is the probability a given location is reached first by a
...
(they \gc2{occupy a larger area than new mutations, since they get a
headstart}).
...
\gc2{As expected intuitively,} the contribution of the rarer mutation decreases as the
...
but it is simpler to note that if the converse is true \gc2{(i.e. that
...
These two effects \gc2{approximately} cancel each other out leading to no strong effect of $s_b$ on the number of independent alleles. 
...
migration and drift, 
potentially leading to a \gc2{within population} signal of a sweep from multiple independent
...
deleterious before the environment switches. \gc2{ In contrast, if they adapt via common variation even distant populations could have
...
However, although geographic lineage sorting via speciation and extinction 
...
e.g.\ if they only offer a local advantage or if they are selective excluded, 
may allow Dobzhansky-Muller incompatibilities to arise within species, 

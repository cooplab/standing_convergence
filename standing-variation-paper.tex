...
selected alleles arise and spread locally.  See Figure
\ref{fig:sim-spread}. However, if the geographic
area is large enough multiple selected alleles can
arise independently and spread before any one has spread across all of
space. Therefore, one allele's rapid spatial spread can be halted by
running into convergent allele.A somewhat analogous situation also
...
\gc{We assume that all alleles of the mutated type are selectively
  ...
  just that all of the mutated type are selective equivalent. As such,
...
\gc{In geographic areas already
 occupied by the mutant type (i.e. in regions were the beneficial
 allele has spread to), new mutations are selectively excluded,
...
Our point $x$ at time $t$ is not yet adapted because no successful alleles appear within this grey cone.
}
  \label{fig:cartoon}
\end{figure}
...
L^2$ and carved out a cylinder in space-time with this circle as a base and
height $L/v$ (the volume of this cylinder is $\pi L^2 \times (L/v)$)
...
successful mutations. Since we assume that \mfp{our successful alleles} arise as a Poisson process, 
in two dimensions
...
occur at all or populations may adapt via some other means (i.e. by using a

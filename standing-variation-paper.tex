...
\author{
Graham Coop$^{*,1}$ \\ email: gmcoop@ucdavis.edu 
\and 
Peter L. Ralph$^{*,2}$ \\ email: pralph@usc.edu  
}
...
$^1$ Center for Population Biology \& Department of Evolution and Ecology \\ 
...
There are an increasing number of examples known in which
different populations of a single species have
...
despite shared selection pressures \citep{}; 
in other cases independent adaptations are identical down the same nucleotide change. 
...
\plr{The next sentence should be more specific (give example?) or maybe be deleted, I think?}
...
In \citep{ralphcoop2010} we formulated a simple model of parallel adaptation in a geographic setting
...
On the basis of this we argue, in the discussion, 
...
i.e.\ the mean number of nut mutants per unit area and per generation that
...
Such a random tesselation of space is known as a Poisson-Voronoi tesselation \citep{poissonvoronoi}
(i.e.\ the cells formed by assigning points to the nearest point in a Poisson process).
The properties of this tesselation by alleles is
...
the cells formed by a Voronoi tesselation have straight sides,
...
The mean spatial density of successful standing variants at $t=0$ (lightning bolts) is $\lambda_0$,
and the mean density in space $\times$ time of successful new mutations (stars) is $\lambda$.
\plr{Add $t=0$ and direction of time to the figure.}
}
  \label{fig:cartoon}
\end{figure}
...
Three G6PD deficiency alleles known to confer resistance to malaria 
...
Assuming that the underlying causes of this drop in fitness have not changed, 
we estimate that $s_d$ has to be roughly XXX to XXX, 
in order to have resulted in the equilibrium frequency seen today. 
...
characteristic length increases until it reaches that predicted by new mutation alone. 
...
A larger selective advantage acts in two conflicting
...
\plr{perhaps we should note: (This can be shown in general by differentiating \eqref{eqn:defines_chi} with respect to $z=\sqrt{s_b}$,
...
and the dependence on $s_b$ when only new mutations contribute is much weaker ($s_b^{-1/6}$, equation \eqref{eqn:chi_new})
...
800km, especially for the lower population density. 
...
compared to the corresponding quantity without standing variation (from \cite{ralphcoop2010}) 
and to the quantity only considering standing variants (i.e.\ $\sqrt{\lambda_0}$).
The other parameters are as estimated from G6PD,
$\mu = 10^{-8}$ and $s_b = XXX$???.
} \label{Fig-G6PD-charlength}
\end{center}
\end{figure}
...
holding the prior disadvantage of the allele fixed at $s_d=0.05$ and the mutation rate fixed at $\mu=10^{-8}$ (right???).  
We compare this to the corresponding quantity without standing variation (from \cite{ralphcoop2010}) 
and to the quantity only considering standing variants (e.g.\ $\sqrt{\lambda_0}$).
...
\end{equation}
and since $\E[\tau] = \int_0^\infty \P\{ \tau > t \} dt$,
\begin{align}
...
\end{align}
\plr{note: I added a 2 to $\lambda_j$.}
...
\begin{equation} \label{eqn-prop-space-allele-j}
    p_j = \int_0^\infty  \left( 2 \lambda_{0,j} \pi v^2 t + \lambda_j \pi v^2 t^3 \right)  
    \; \exp \left( - \sum_k ( \lambda_{0,k} \pi v^2 t^2 + \lambda_k \pi v^2 t^3 / 3 ) \right) dt .
\end{equation}
...
at which a possible mutation is free of negative pleiotropy (call this class $1$);
...
and think of the second class as ten, a hundred, or one thousand base pairs. 
...
{\bf Proportion of space covered by rarer but less pleiotropic
  ...
  \plr{add other parameter values}
...
and also compare the results above to the results without geographic structure of \citet{softsweeps}.
...
then $\sigma^2 \rho = Z_\tau \approx e^{s_b \tau} W$ \citep{jaegers1975branching}.
...
Allowing standing variation in our model, unsurprisingly, 
increases the extent of convergence among populations.
Importantly, it may greatly lower the time until the species becomes adapted 
...
Given sufficient time these haplotypes will mix together, 
leading to a signal of a sweep with multiple mutations if our selected allele occurs at the same locus, 
or to multiple partial sweeps if the loci are scattered across the genomes.
...
However, we currently lack good methods with which to test this.
...
even if gene flow is not high enough to prevent neutral differentiation, 
...
and will allow us to form an improved picture of the relationship 
between the level of neutral population structure, 
and the age and geographic extent of spread of selected alleles.
...
\citep[see ][for work on geographic hitchhiking]{barton2013genetic}. 
The overall rate of coalescence
...
Drift alone will slowly sort variants within species. 
...
but that the majority of incipient species do not persist long due to readsorption or extinction. 
...
\plr{I don't understand the following (but haven't read gillespie '94).}

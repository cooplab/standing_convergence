...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=\textwidth]{spreading_alleles_trimmed}
  \end{center}
  \caption{
...
spread locally from both standing variants (lightning bolts) and new mutations (stars). 
...
so the space-time profile of their spread forms a cone.
...
The grey dashed line shows a cone radiating backward in space-time,
originating from a point $(x,t)$, with slope $v$, its radius at $t=0$ is $v t$. 
...
we estimate that $s_d$ is upward of $0.05$ (if $sb \geq 0.05$)
in order to have resulted in the equilibrium frequency seen today in areas with endemic malaria. 
...
standing variants still makes up quite large proportion of the adaptive alleles, 
and an even larger proportion of the range 
(they get to occupy a larger area than new mutations, since they start earlier).
...
the first allele would have an selective advantage $\delta s =s_{b,1}-s_{b,2}$,
...
These two effects cancel each other out leading to no strong effect of $s_b$ on the number of independent alleles. 
...
this relationship differs markedly from the case of multiple competing alleles in a panmictic population \citep{softsweepsI,softsweepsII}. 
...
Therefore, it seems possible that populations within species may be tending towards speciation, 
